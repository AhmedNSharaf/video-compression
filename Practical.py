...
def huffman_codebook(freqs):
    ...
    return build_huffman_codes(tree)
...
class VideoCompressorApp:
    ...
    def compress_video_thread(self):
        ...
        self.root.after(0, lambda: self.status_label.config(text=f"Status: Done! Compression Ratio: {compression_ratio:.2f}, PSNR: {psnr:.2f} dB"))
    ...
    def decompress(self, input_path, output_path):
        ...
        for _ in range(frame_count):
            ...
            recon_frame = cv2.cvtColor(recon_frame.astype(np.uint8), cv2.COLOR_YUV2BGR)
    ...
    def block_matching(self, block, ref_block, block_size):
        search_range = 4  # Reduced from 8 to improve speed
        best_mv = (0, 0)
        min_sad = float('inf')
        h, w = block.shape
        ref_h, ref_w = ref_block.shape

        for dy in range(-search_range, search_range + 1, 2):  # Step by 2 for faster search
